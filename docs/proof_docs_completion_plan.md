...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   The implementation route is quotient/rank-nullity, not a black-box
   ...
   * Put the induced symmetric form on `Rperp ⧸ R`.  Its radical is zero:
     if a class represented by `x ∈ Rperp` pairs to zero against every class,
     then `x` pairs to zero against all of `Rperp`; together with the defining
     map `x ∈ Rperp`, nondegeneracy of `N` identifies the annihilator of
     `Rperp` with `R`, so the class is zero.
   ...
   * Package `Q` as `ComplexMinkowskiMaximalIsotropicSubspaceIn d N` and apply
   ...
   The remaining Lean-engineering blocker is the final wrapper proving
   `BHW.complexMinkowski_maximalIsotropicSubspaceIn_extending`: specifically,
   assemble the named candidate's finrank equality with
   `BHW.complexMinkowskiRelativeOrthogonalQuotientPreimage_finrank_eq_add`
   without forcing Lean to unfold the dependent quotient-preimage expression in
   the theorem head.
