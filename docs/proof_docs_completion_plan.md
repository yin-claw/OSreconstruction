...
3. current strict-route frontier: finish the upstream direct OS-I compact
   ...
   `BHW.OS45SourcePatchBHWJostPairData.restrict`,
   ...
   `BHW.OS45SourcePatchBHWJostPairData.difference_holo`,
   `BHW.OS45SourcePatchBHWJostPairData.difference_wick_trace`,
   `BHW.OS45SourcePatchBHWJostPairData.difference_real_trace`, and
   ...
   `BHW.realSourceBranchDifference_zero_of_pairData_difference_zero`, and
   `BHW.integral_realSourceBranchDifference_eq_zero_to_pairing_eq`,
   with the checked source-patch/compact-packet bridges
   ...
   the exact-source-patch consumers
   ...
   checked `BHW.same_sourceOrientedInvariant_detOneOrbit_or_singularLimit`,
   checked `BHW.extendedTube_same_sourceOrientedInvariant_extendF_eq`,
...
direct BHW/Jost pair-data algebra layer.  The checked file
...
`BHW.BHWJostOrientedCertifiedTransferTerminalPointTrace.toTerminalPointTrace`,
`BHW.BHWJostOrientedCertifiedTransferTerminalPointTrace.ofTracePoint`,
`BHW.BHWJostOrientedCertifiedTransferTerminalPointTrace.atEndpoint`,
`BHW.BHWJostOrientedCertifiedTransferTerminalPointTrace.point_mem_terminalLocalChart`,
...
`BHW.BHWOrientedContinuationTraceAtlasData`,
`BHW.BHWOrientedContinuationTraceAtlasData.chainAt`,
...
traces plus terminal-point comparison into the checked source-patch atlas and
glued branch, and turn a produced terminal comparison patch into equality of
continued values.  The chain-atlas
...
same-endpoint comparison to derive the atlas base agreement field.  The
trace-atlas constructor avoids the arbitrary-chain comparison surface
altogether: it observes selected transfer traces at the overlap point, applies
terminal-point trace comparison, rewrites by `branch_eq_local`, and proves
base agreement by comparing against the zero-step base trace observed inside
the initial chart.  The certified-trace variant strengthens the strict route
surface by requiring each selected trace to carry
...
- checked `BHW.same_sourceOrientedInvariant_detOneOrbit_or_singularLimit`, including
...
2026-05-09: the descent/normal-Riemann group and the oriented real-patch
uniqueness group are no longer active theorem-2 frontiers.  They may remain as
checked or conditional side infrastructure, but they must not be extended or
consumed as a theorem-2 route unless an actual OS-paper error is first found
and documented.  The active theorem-2 blocker is the literal OS-II / OS I §4.5
compact Figure-2-4 branch-difference producer
`BHW.os45Figure24_sourcePatch_pairing_eq_swappedSourcePatch_of_OSI45` and the
direct OS45 consumer chain below.  Public theorem-2 Lean remains closed until
that branch-comparison route is implemented without replacing it by
source-variety descent.
...
production Lean in `SourceHWBranchLaw.lean`.  The summary ledger must not
describe the high-rank Witt repair, low-rank selected block, residual-frame
alignment, dual-frame construction, contraction family, or singular two-curve
limit as anonymous "proof-doc-only" gaps.  They are checked support statements,
with the active low-rank path passing through the separate-frame producer
rather than the optional common-frame determinant-repair branch.  A future Lean
pass should now start after this group, for example at the descent/removability
gate, not at `BHW.extendF_complexLorentzInvariant_of_cinv`, the
coefficient-kernel bridge,
...
`BHW.sourceRealFullFrameDetLinePolynomial_eval`,
   ...
   its origin source/target membership are checked as side support.  If this
   real/full-frame side infrastructure is ever resumed as coordinate support,
   the final producer may not combine an independently chosen real inverse with
   the complex max-rank chart.  The remaining producer packaging must instead
   restrict the same complex IFT chart used by
   `sourceFullFrameRealCompatibleMaxRankChartDataRaw` to the conjugation-stable
   real slice, rebuild the real gauge-slice packet from that common chart, and
   only then assemble the selected-frame source patch and open-image field.
   The
...
source-patch theorem.  Production Lean has now checked the algebraic consumer
chain below the analytic producer:
`BHW.os45_pairData_difference_identity_of_wickDistributionZero`,
`BHW.os45_pairData_difference_realTrace_zero_of_wickDistributionZero`,
`BHW.os45CompactFigure24WickPairingEq_of_pairData_difference_zero`,
`BHW.os45CompactFigure24WickPairingEq_family_of_pairData_difference_zero`,
`BHW.sourceDistributionalAdjacentTubeAnchor_of_pairData_difference_zero`,
and the selected-adjacent wrapper
`BHW.bvt_F_distributionalJostAnchor_of_pairData_difference_zero`.  Therefore
the next hard Lean target is not a new `DifferenceData` wrapper; it is the
direct OS-I/BHW-Jost construction of
`P : BHW.OS45SourcePatchBHWJostPairData ... V` together with Wick-side compact
zero for `P.τ` on that same `V`, using the now-checked Wick pairing
integrability support consumed by
`BHW.os45_wickBranchDifference_zero_of_euclideanEdge_pairing_eq_on_timeSector`,
plus the now-checked real-source `extendF` integrability support.  The compact
...
Figure-2-4 real source trace, and the checked pair-data algebra packages the
result.  It may not be replaced by separate real-branch `= OS.S` statements,
half-time common-edge continuity of raw `extendF`,
...
0. Dimension and analytic-space audit: Hall-Wightman is printed for
   ...
   `same_sourceOrientedInvariant_detOneOrbit_or_singularLimit` (checked);
   `extendedTube_same_sourceOrientedInvariant_extendF_eq` (checked);
   ...
   The small-arity max-rank reducer and the full max-rank dispatcher are now
   checked as
   `BHW.sourceComplexGramRegularAt_of_HWSourceGramMaxRankAt_any`,
   `BHW.sourceOrientedExtendedTubeLocalRealizationData_of_smallArityMaxRank`,
   and `BHW.sourceOrientedExtendedTubeLocalRealizationData_of_maxRank`.
   The full-frame proof uses `isOpen_extendedTube`, pulls an open model
   ...
   `toOriginal` map, `BHW.SourceOrientedResidualPolydiscData` with
   ...
   theorem surfaces are `sourceMatrixMinor_sourceMinkowskiGram_fullFrame`,
   `sourceOrientedAlgebraicRelations`,
   `sourceOrientedAlgebraicVariety`,
   `sourceOrientedMinkowskiInvariant_mem_algebraicRelations`,
   `sourceOrientedGramVariety_subset_algebraicVariety`,
   `isClosed_sourceOrientedAlgebraicVariety`,
   ...
   The forward actual-tuple Cauchy-Binet equation is checked in
   `SourceOrientedCauchyBinet.lean`: set
   `Mι := sourceFullFrameMatrix d n ι z` and
   `Mκ := sourceFullFrameMatrix d n κ z`, rewrite the selected source Gram
   minor as `det (Mι * ηℂ * Mκᵀ)`, then apply `Matrix.det_mul`,
   `Matrix.det_transpose`, and `minkowskiMetricDet`.  This proves the
   source-variety-to-algebraic relation direction for the determinant
   equation.  The selected-frame alternation relation is also a small
   actual-tuple theorem: `sourceFullFrameDet_reindex_selectedFrame` is
   `Matrix.det_permute` for the row-permuted selected full-frame matrix.
   `SourceOrientedAlgebraicRelations.lean` now checks the explicit algebraic
   relation set itself: `sourceOrientedAlgebraicRelations` is
   `G.gram ∈ sourceSymmetricRankLEVariety n (d + 1)` plus ordered-frame
   alternation and the Cauchy-Binet equations;
   `sourceOrientedMinkowskiInvariant_mem_algebraicRelations` combines the
   checked pure-Gram rank inclusion, determinant alternation, and
   Cauchy-Binet; `sourceOrientedGramVariety_subset_algebraicVariety` is range
   elimination; and `isClosed_sourceOrientedAlgebraicVariety` is the finite
   closed-intersection proof.  These are not the invariant-ring kernel theorem
   and do not prove `sourceOrientedGramVariety_eq_algebraic`.
   ...
   the strict lower-rank inequality.  The determinant-zero-locus bridge
   ...
   real-locus equation are now checked, and the specialized constructor
   `sourceFullFrameRealGaugeSliceData_of_frameKernelCoord_realExtension`
   supplies a useful checked support packet from the frame-domain data.  That
   packet is not a theorem-2 critical-path chart under the strict-route reset;
   if this side infrastructure is resumed, it must be rederived from the
   common complex restriction below.  The open-image obligation belongs to
   the final shrunken source chart, not to the frame-domain packet: the next
   side-infrastructure frontier would be
   ...
   Route-integrity note, 2026-05-09: this checked Lean surface records the
   historical support packet built from the independently chosen real IFT chart.
   It must not be consumed as a theorem-2 critical-path ingredient under the
   strict OS-II route.  If this side infrastructure is resumed, the final packet
   has to be rebuilt as
   `sourceFullFrameRealSelectedFrameProductChartData_of_complexRestrictedSlice`,
   whose real partial homeomorphism is the restriction of the same complex
   chart used in `sourceFullFrameRealCompatibleMaxRankChartDataRaw`.
   ...
   The openness layer is now specified in the blueprint at Lean-transcription
   granularity.  It must be implemented in this order:

   1. Name the raw ambient domain
      `sourceFullFrameRealCompatibleChartDomainAmbient`, prove
      `sourceFullFrameRealCompatibleChartDomain_eq_ambient`,
      `sourceFullFrameRealCompatibleChartDomainAmbient_open`, and then
      `sourceFullFrameRealCompatibleChartDomain_relOpen`.  The only
      non-total-continuity point is the final gauge-slice source condition,
      handled by `eK.continuousOn_symm` on `eK.target` plus
      `continuousOn_iff'.mp`.
   2. Name the model-side open predicate
      `sourceFullFrameRealCompatibleModelChartOpenDomain` and prove the two
      readoff formulas for `chartInvRaw y`: selected symmetric coordinate
      equals the gauge-slice symmetric map at `F.complexCoordEquiv y.1`, and
      selected kernel source coordinate equals `y.1` whenever `y.1 ∈ eK.source`.
   3. Prove
      `sourceFullFrameRealCompatibleModelChartDomain_eq_openDomain`, then
      `sourceFullFrameRealCompatibleModelChartDomain_open` as the intersection
      of the kernel source, symmetric-chart source, and reconstructed-frame
      determinant-nonzero open conditions.
   4. Use
      `LocalBiholomorphOnSourceOrientedVariety.inv_image_open_relOpen` with
      `sourceFullFrameRealCompatibleChartDomain_relOpen` and the checked raw
      local biholomorph to prove
      `sourceFullFrameRealCompatibleShrunkenChartDomain_relOpen`; the already
      checked image equality supplies `chart_open`, and
      `sourceFullFrameRealCompatibleShrunkenChartDomain_subset_rawDomain`
      supplies max-rank.

   Lean checkpoint, 2026-05-09: items 1--4 are now checked, including the
   finite-coordinate `C` producer
   `sourceOrientedFullFrameMaxRankChartData_of_realCompatibleSlice`.  The
   remaining blocker is sharper than the previous plan stated: the final
   finite-coordinate real formula cannot be obtained merely by comparing the
   existing real and complex `toOpenPartialHomeomorph` charts.  Those local
   inverse charts are chosen independently by Mathlib, so their inverses are
   not automatically equal on complexified real coordinates.

   Before implementing `sourceFullFrameRealCompatibleImplicitChartData`, the
   blueprint must close the real-locus compatibility bridge by a single fixed
   route: restrict the same complex IFT chart used by
   `sourceFullFrameRealCompatibleMaxRankChartDataRaw` to a
   conjugation-stable real slice.  Do not compare the independently chosen real
   and complex `toOpenPartialHomeomorph` inverses.

   The required theorem order is:

   1. `sourceFullFrameRealCompatibleNormalizedKernelMap_conj`: the complex
      normalized kernel map commutes with coordinatewise conjugation.  This
      must be proved algebraically, not by analytic continuation.  First prove
      `sourceFullFrameRealSlice_complexCoordEquiv_conj`,
      `sourceFullFrameRealSliceKernelCoordEquiv_conj`, and
      `sourceFullFrameRealSliceKernelCoordEquiv_symm_conj` by decomposing an
      arbitrary finite complex coordinate into coordinatewise real and
      imaginary parts, using complex linearity, `F.complexCoordEquiv_real_eq`,
      and the checked `sourceFullFrameRealSliceKernelCoordEquiv_real_im`.
      Then prove
      `sourceFullFrameGaugeSliceMapSymmetric_conj`,
      `sourceFullFrameRealCompatibleKernelProjection_conj`, and
      `sourceFullFrameRealCompatibleExplicitKernelMap_conj` from the explicit
      polynomial/full-frame formulas, determinant conjugation, the real base
      `M0R.map Complex.ofReal`, and the determinant-direction projection
      formula.  The final normalized-map theorem is obtained by unfolding and
      applying `sourceFullFrameRealSliceKernelCoordEquiv_symm_conj`.
   2. `sourceFullFrameRealCompatibleNormalizedKernelConjStableOpenPartialHomeomorphC`:
      restrict `sourceFullFrameRealCompatibleNormalizedKernelOpenPartialHomeomorphC`
      by `OpenPartialHomeomorph.restrOpen` to the open set
      `eK.source ∩ conj ⁻¹' eK.source`, with openness from
      `eK.open_source` and continuity of coordinatewise conjugation.  Record
      the source-unfolding theorem so later membership proofs use the intended
      stable-source shape directly.
   3. `sourceFullFrameRealCompatibleNormalizedKernelConjStable_real_inverse`:
      if a real complexified point lies in the stable target, then its complex
      inverse is real.  Use conjugation equivariance and injectivity on the
      stable source.
   4. `sourceFullFrameRealCompatibleNormalizedKernelOpenPartialHomeomorph_fromComplex`:
      build the real open partial homeomorphism whose source/target are the
      real preimages of the stable complex source/target and whose map/inverse
      are the real parts of the stable complex map/inverse.
   5. Prove the coe and inverse bridge:
      `sourceFullFrameRealCompatibleNormalizedKernelOpenPartialHomeomorph_fromComplex_coe`
      and
      `sourceFullFrameRealCompatibleNormalizedKernelOpenPartialHomeomorph_fromComplex_symm_realToComplex`.
   6. Rebuild the selected-frame real slice packet from that restricted chart:
      `sourceFullFrameRealGaugeSliceData_of_complexRestrictedKernelCoord` and
      `sourceFullFrameRealSelectedFrameProductChartData_of_complexRestrictedSlice`.
   7. Prove the real-invariant selected-kernel source-coordinate formula:

   ```lean
   theorem
       BHW.sourceFullFrameRealCompatibleSelectedKernelSourceCoordC_realInvariant_eq
       ... (x : Fin n -> Fin (d + 1) -> ℝ)
       (hframe :
         BHW.sourceRealFullFrameMatrix d n ι x ∈
           BHW.sourceFullFrameRealCompatibleFrameDomain d hM0R F)
       (hchart :
         BHW.sourceRealOrientedMinkowskiInvariant d n x ∈
           (BHW.sourceFullFrameRealCompatibleMaxRankChartDataRaw
             ι hM0R F).Ω) :
       BHW.sourceFullFrameRealCompatibleSelectedKernelSourceCoordC
           ι hM0R F
           (BHW.sourceRealOrientedMinkowskiInvariant d n x) =
         let S :=
           BHW.sourceFullFrameRealGaugeSliceData_of_complexRestrictedKernelCoord
             d hM0R F
   ...
   After these seven steps, the finite-coordinate chart formula and final
   `SourceFullFrameRealCompatibleImplicitChartData` constructor are mechanical
   side-infrastructure consumers.  The older
   `sourceFullFrameRealGaugeSliceData_of_frameKernelCoord_realExtension` may
   remain as checked support, but it is not a theorem-2 critical-path packet
   under the strict-route reset unless this side infrastructure is explicitly
   resumed and rederived from the common complex restriction above.
...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   `BHW.sourceFullFrameBasis_apply`,
	   ...
	   `BHW.sourceOriented_soCompatible_of_sameInvariant`,
	   `BHW.complexLorentzGroupOfLinearEquivPreservesInner`,
	   `BHW.complexLorentzVectorAction_ofLinearEquivPreservesInner`,
	   `BHW.sourceFullFrameMap_complexLorentzVectorAction_of_det_one`,
	   `BHW.sourceFullFrameMap_complexLorentzOrbit_of_det_one`,
	   `BHW.sourceOriented_fullRank_detOneOrbit_of_sameInvariant`,
   ...
   `BHW.HWLowRankResidualFrameExtensionData`,
   ...
   `BHW.complexMinkowski_dualPair_linearIndependent_left`,
   `BHW.complexMinkowski_dualPair_linearIndependent_right`,
   `BHW.complexMinkowski_span_frame_disjoint_selected_of_dual_orthogonal`,
   `BHW.complexMinkowski_span_dualFrame_disjoint_selected_sup_frame`,
   `BHW.complexMinkowski_span_frame_disjoint_dualFrame`,
   `BHW.submoduleScaleLinearEquiv`,
   `BHW.directSum_scale_sup_equiv`,
   `BHW.directSum_scale_sup_equiv_apply_of_eq_add`,
   `BHW.complexMinkowski_hyperbolicFrameSpan_nondegenerate`,
   `BHW.complexMinkowski_selected_sup_hyperbolicFrameSpan_nondegenerate`,
   `BHW.complexMinkowski_hyperbolicFrameSpanContraction`,
   `BHW.complexMinkowski_hyperbolicFrameSpanContraction_maps_q`,
   `BHW.complexMinkowski_hyperbolicFrameSpanContraction_maps_qDual`,
   `BHW.complexMinkowski_hyperbolicFrameSpanContraction_preserves`,
   `BHW.directSum_congr_sup_equiv`,
   `BHW.directSum_congr_sup_equiv_apply_of_eq_add`,
   `BHW.complexMinkowski_hyperbolicFrameSpan_orthogonal_selected`,
   `BHW.complexMinkowski_selected_disjoint_hyperbolicFrameSpan`,
   `BHW.complexMinkowski_selectedHyperbolicContraction`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_maps_M`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_maps_q`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_maps_qDual`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_preserves`,
   `BHW.directSum_scale_sup_equiv_det`,
   `BHW.directSum_congr_sup_equiv_det`,
   `BHW.complexMinkowski_hyperbolicFrameSpanContraction_det`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_det`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_globalLinearEquiv`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_globalLinearEquiv_apply_block`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_globalLinearEquiv_maps_M`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_globalLinearEquiv_maps_q`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_globalLinearEquiv_maps_qDual`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_globalLinearEquiv_preserves`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_globalLinearEquiv_det`,
   `BHW.complexMinkowski_selectedHyperbolicContractionFamily`,
   `BHW.complexMinkowski_selectedHyperbolicContractionFamily_vectorAction`,
   `BHW.complexMinkowski_selectedHyperbolicContractionFamily_maps_M`,
   `BHW.complexMinkowski_selectedHyperbolicContractionFamily_maps_q`,
   `BHW.complexMinkowski_selectedHyperbolicContractionFamily_maps_qDual`,
   `BHW.ComplexMinkowskiMaximalIsotropicSubspaceIn`,
   `BHW.ComplexMinkowskiMaximalIsotropicFrameIn`,
   `BHW.complexMinkowskiMaximalIsotropicFrameIn_of_subspace`,
   ...
   `BHW.complexMinkowski_totallyIsotropic_embedding_into_maximalFrame`,
   `BHW.directSum_identity_sum_isotropicMaximalFrameEmbedding`,
   `BHW.HWLowRankResidualFrameExtensionData`,
   `BHW.hw_lowRank_residualAlignmentData_of_frameExtensionData`,
   `BHW.HWLowRankCommonIsotropicFrameData`,
   `BHW.HWLowRankResidualAlignmentData`,
   `BHW.hw_lowRank_commonIsotropicFrameData_of_residualAlignmentData`,
   `BHW.complexLorentzAction_residualExpansion`,
   `BHW.hw_lowRank_isotropicNormalForm_of_commonFrameData`,
   `BHW.hw_sameSourceGram_singular_contractionData_of_commonFrameData`,
   ...
   `q` by `Real.exp (-t)` and `qDual` by `Real.exp t`, and combine it with
   the identity on `M`.  The selected-plus-hyperbolic block equivalence,
   its formulas on `M`, `q`, and `qDual`, and its form-preservation theorem
   are now checked as
   `BHW.complexMinkowski_selectedHyperbolicContraction`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_maps_M`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_maps_q`,
   `BHW.complexMinkowski_selectedHyperbolicContraction_maps_qDual`, and
   `BHW.complexMinkowski_selectedHyperbolicContraction_preserves`.  The
   determinant and global packaging layer is now also checked:
   `BHW.complexMinkowski_selectedHyperbolicContraction_det` computes the
   selected-plus-hyperbolic determinant from the reciprocal exponential
   diagonal blocks; `BHW.complexMinkowski_selectedHyperbolicContraction_globalLinearEquiv`
   splits the ambient space as this nondegenerate block plus its orthogonal
   complement and extends by identity; `_preserves` and `_det` prove the
   global map is form-preserving and determinant one; and
   `BHW.complexMinkowski_selectedHyperbolicContractionFamily` packages it as
   a `ComplexLorentzGroup d` with checked action formulas on `M`, `q`, and
   `qDual`.  No general degenerate subspace-isometry extension is used in this
   low-rank boost.
   ...
   the remaining geometric producer
   `BHW.hw_lowRank_residualFrameExtensionData_of_selectedAlignment` must
   supply `BHW.HWLowRankResidualFrameExtensionData A`: a compatible maximal
   frame in `A.Mᗮ`, a determinant-one Lorentz transformation fixing the
   selected span, and span membership for the corrected left residuals and the
   right residuals.  The checked theorem
   `BHW.hw_lowRank_residualAlignmentData_of_frameExtensionData` converts this
   to `BHW.HWLowRankResidualAlignmentData A`; then
   `BHW.hw_lowRank_commonIsotropicFrameData_of_residualAlignmentData` extracts
   coefficient functions and proves the two endpoint expansion equations,
   yielding `BHW.HWLowRankCommonIsotropicFrameData A`.  The checked normal-form
   consumer then constructs the dual frame, boost family, tube membership, and
   convergence fields.
   The global maximal-frame existence layer is checked in
   `BHWPermutation/SourceHWMaximalIsotropicFrame.lean`:
   ...
   dimension and maximality bound.  The wrappers
   `BHW.complexMinkowski_totallyIsotropic_embedding_into_maximalFrame` and
   `BHW.directSum_identity_sum_isotropicMaximalFrameEmbedding` feed this bound
   directly into the checked residual embedding/direct-sum packet.  The final
   geometric landing surface is now
   `BHW.HWLowRankResidualFrameExtensionData`; the checked conversion
   `BHW.hw_lowRank_residualAlignmentData_of_frameExtensionData` turns it into
   `BHW.HWLowRankResidualAlignmentData`.  This does
   not yet solve the residual-alignment
   problem, because the route still needs a determinant-one correction moving
   the left residual span into the compatible frame span.  The
   frame-compatibility theorem is now checked:
   `BHW.complexMinkowski_maximalIsotropicFrameIn_extending` extends a given
   totally isotropic subspace `R ≤ N` to a globally maximal totally isotropic
   frame with `R ≤ Submodule.span ℂ (Set.range F.q)`.  The older checked
   ...
   checked as `BHW.complexMinkowskiMaximalIsotropicFrameIn_of_subspace_span_eq`
   ...
   The checked implementation route is quotient/rank-nullity, not a black-box
   degenerate Witt-extension call:

   * Work inside the nondegenerate ambient subspace `N` with restricted form
     `B := (sourceComplexMinkowskiBilinForm d).restrict N`.
   * For the given totally isotropic `R ≤ N`, use the checked subtype helper
     `BHW.complexMinkowskiSubmoduleIn N R : Submodule ℂ N` and define the
     relative orthogonal by the checked
     `BHW.complexMinkowskiRelativeOrthogonalIn N
       (BHW.complexMinkowskiSubmoduleIn N R)`.  The checked theorem
     `BHW.complexMinkowskiSubmoduleIn_le_relativeOrthogonalIn_of_totallyIsotropic`
     proves the isotropic-subspace inclusion into this relative orthogonal.
     The checked
     `BHW.complexMinkowskiSubmoduleInRelativeOrthogonal` and
     `BHW.complexMinkowskiSubmoduleIn_comap_le_relativeOrthogonalIn_restrict_ker`
     identify the retyped subspace inside `Rperp` and prove the kernel
     condition needed for the quotient induced form.
   * Put the induced symmetric form on `Rperp ⧸ R`.  The quotient radical
     bridge is checked separately, but the containment-compatible maximal-frame
     wrapper does not need a nondegeneracy hypothesis: maximal totally isotropic
     subspaces exist for any finite-dimensional bilinear form, and the
     rank-nullity comparison below is the global maximality argument used by the
     final checked theorem.
   * Choose a maximal totally isotropic subspace `Qbar` of this quotient by the
     existing finite `Nat.findGreatest` pattern, but for the quotient induced
     form.  Let `Q` be the preimage of `Qbar` in `Rperp`, viewed back inside
     `N`.  Then `R ≤ Q`, `Q ≤ N`, and `Q` is totally isotropic.
   * Global maximality of `Q` follows by the explicit dimension comparison.
     For any totally isotropic `S ≤ N`, define
   ...
   * Package `Q` as `ComplexMinkowskiMaximalIsotropicSubspaceIn d N` and apply
   ...
   The Lean support now includes the local subtype/relative-orthogonal helpers,
   the kernel condition for quotient descent, the induced quotient bilinear
   form `BHW.complexMinkowskiRelativeOrthogonalQuotientForm`, its `mk` and
   symmetry lemmas, and the retyping finrank identities
   `BHW.complexMinkowskiSubmoduleIn_finrank_eq_of_le`,
   `BHW.complexMinkowskiSubmoduleInRelativeOrthogonal_finrank_eq_of_le`, and
   `BHW.complexMinkowskiSubmoduleInRelativeOrthogonal_finrank_eq_of_totallyIsotropic`.
   The quotient radical bridge is also checked as
   `BHW.complexMinkowskiRelativeOrthogonalQuotientForm_nondegenerate_of_restrict_ker_le`:
   once the kernel of the restricted form on `Rperp` is shown to lie in the
   retyped `R`, the quotient form on `Rperp / R` is nondegenerate.  The
   double-orthogonal computation supplying that inclusion is now checked as
   `BHW.complexMinkowskiRelativeOrthogonalIn_eq_orthogonal`,
   `BHW.complexMinkowskiRelativeOrthogonal_restrict_ker_le_submoduleInRelativeOrthogonal`,
   and
   `BHW.complexMinkowskiRelativeOrthogonalQuotientForm_nondegenerate`.
   The first quotient-preimage carrier packet is now checked in
   `SourceHWMaximalIsotropicQuotient.lean` as
   ...
   the packaged forms are checked as
   `BHW.BilinFormMaximalTotallyIsotropicSubspace`,
   `BHW.bilinFormMaximalTotallyIsotropicSubspace`,
   `BHW.ComplexMinkowskiRelativeOrthogonalQuotientMaximalPackage`, and
   `BHW.complexMinkowskiRelativeOrthogonalQuotient_maximalPackage`.
   ...
   `BHW.complexMinkowskiPairingKernelQuotientImage_isotropic`; its concrete
   dimension and maximal-quotient bound are now checked as
   `BHW.complexMinkowskiPairingKernelQuotientImage_finrank_add_inter` and
   `BHW.complexMinkowskiPairingKernelQuotientImage_finrank_le_of_maximal`.
   The generic
   ...
   The `finrank T` cancellation is now checked for an arbitrary candidate
   subspace by
   `BHW.complexMinkowskiSubspace_finrank_le_of_quotient_maximal` and
   `BHW.complexMinkowskiSubspace_global_maximal_of_quotient_maximal`.
   The final candidate carrier fields are checked in
   `SourceHWMaximalIsotropicExtension.lean` as
   `BHW.complexMinkowskiRelativeOrthogonalQuotientPreimageOfMaximal`,
   `BHW.complexMinkowskiRelativeOrthogonalQuotientPreimageOfMaximal_contains`,
   `BHW.complexMinkowskiRelativeOrthogonalQuotientPreimageOfMaximal_le`, and
   `BHW.complexMinkowskiRelativeOrthogonalQuotientPreimageOfMaximal_isotropic`.
   The containment-compatible maximal extension is checked in
   `SourceHWMaximalIsotropicFinal.lean` as
   `BHW.complexMinkowski_maximalIsotropicSubspaceIn_extending` and
   `BHW.complexMinkowski_maximalIsotropicFrameIn_extending`.  The proof
   intentionally assembles the raw quotient-preimage candidate directly, then
   packages its basis frame, rather than unfolding the irreducible named
   candidate inside the finrank theorem head.
   This common-frame producer should not be revived by adding a new generic
   `complexMinkowski_wittExtension_subspaceIsometry` theorem.
   It is retained as optional route history; the active OS-II path now uses
   the checked separate-frame contraction in `SourceHWSeparateResidualFrame.lean`.
   The residual-frame alignment side route has an
   ...
   converts this packet into the required proper Lorentz correction, and the
   checked def
   `BHW.hw_lowRank_residualFrameExtensionData_of_blockExtensionData` packages
   that correction with a compatible maximal frame into
   `BHW.HWLowRankResidualFrameExtensionData`.  The next implementation theorem
   should therefore either produce this proper-branch block data under an
   explicit properness hypothesis or produce the direct ambient determinant-one
   equivalence for the full-block branch; it should not wrap the final `Λfix`
   conclusion without discharging one of those two determinant cases.
   The first hyperbolic-completion sublemma is now checked as a
   smaller, Lean-level data packet:
   `BHW.ComplexMinkowskiIsotropicSubspaceBasisDualFrameData d N R`.  Given a
   nondegenerate ambient subspace `N`, an inclusion `R ≤ N`, and
   `ComplexMinkowskiTotallyIsotropicSubspace d R`, it stores
   `k = Module.finrank ℂ R`, a basis frame
   `frame : Fin k → Fin (d + 1) → ℂ` with
   `Submodule.span ℂ (Set.range frame) = R`, an isotropic dual frame
   `dual : Fin k → Fin (d + 1) → ℂ` in `N`, and the exact pairings
   `⟪frame c, frame c'⟫ = 0`, `⟪dual c, dual c'⟫ = 0`, and
   `⟪frame c, dual c'⟫ = if c = c' then 1 else 0`.  Its producer
   `BHW.complexMinkowski_isotropicSubspace_basisDualFrameData` is checked in
   `SourceHWResidualHyperbolicCompletion.lean` from `Module.finBasis ℂ R` plus
   `BHW.complexMinkowski_isotropicFrame_dualFrameIn`.  The same file checks the
   immediate consumers
   `BHW.ComplexMinkowskiIsotropicSubspaceBasisDualFrameData.frame_orthogonal_to_subspace`,
   ...
   are available.  This branch split is no longer the active low-rank
   producer: the checked separate-frame route avoids the common target
   completion and full-ambient determinant-one obstruction entirely.
   ...
   `(hR_orth : ∀ x : R, ∀ m : M,
   sourceComplexMinkowskiInner d (x : Fin (d + 1) -> ℂ)
   (m : Fin (d + 1) -> ℂ) = 0)` and concludes
   `R ≤ complexMinkowskiOrthogonalSubmodule d M`;
   `BHW.complexMinkowski_disjoint_of_nondegenerate_orthogonal` has the same
   orthogonality hypothesis plus
   `ComplexMinkowskiNondegenerateSubspace d M` and concludes `Disjoint M R`;
   and `BHW.hw_lowRank_residualSubspaces_orthogonalComplement_after_selectedAlignment`
   packages the two aligned residual spans with their orthogonal-complement
   inclusions, disjointness from `A.M`, and total-isotropy fields.  These
   lemmas feed the checked
   `BHW.isCompl_comap_sup_subtype_of_disjoint` and
   `BHW.directSum_identity_sum_isotropicEmbedding`, which build the actual
   linear equivalence
   `↥(M ⊔ R) ≃ₗ[ℂ] ↥(M ⊔ LinearMap.range E)` for the identity on `M` plus
   an injective residual embedding.  The checked theorem
   `BHW.directSum_identity_sum_isotropicEmbedding_preserves` proves the
   block-pairing preservation from the residual embedding field
   `hE_preserves`; this support layer does not assert the maximal isotropic
   frame or the null-boost family.
   The residual-summand behavior is also checked as
   `BHW.directSum_identity_sum_isotropicEmbedding_maps_right`: for
   `x : R`, the direct-sum equivalence sends
   `⟨(x : Fin (d + 1) -> ℂ), Submodule.mem_sup_right x.2⟩`
   to the underlying vector `E x` in the target sup.  Symmetrically,
   `BHW.directSum_identity_sum_isotropicEmbedding_maps_left` proves that
   selected-span vectors are fixed.  The preservation theorem decomposes
   arbitrary sup-submodule vectors into those two summands and expands the
   four bilinear block terms.
   The residual embedding step below a chosen frame is also checked:
   `BHW.complexMinkowski_totallyIsotropic_embedding_into_frame` takes an
   independent totally isotropic frame `q`, a totally isotropic residual
   subspace `R`, and a dimension bound `Module.finrank ℂ R <= s`; it returns
   an injective linear map `E : R ->ₗ[ℂ] Fin (d + 1) -> ℂ` whose range lies in
   `Submodule.span ℂ (Set.range q)` and whose residual pairings agree with
   those on `R`.  The checked
   `BHW.coefficients_of_family_mem_span_finite_frame` then extracts finite
   coefficient functions for any source-indexed residual family lying in that
   frame span; this is the mechanical source of the eventual `aLeft` and
   `aRight` normal-form fields.  The checked
   `BHW.directSum_identity_sum_isotropicFrameEmbedding` composes these inputs:
   an independent totally isotropic frame `q`, frame orthogonality to `M`,
	   residual total-isotropy, and the dimension bound `finrank R <= s` produce
	   the injective residual embedding, its `M`-orthogonality, its pairing
	   preservation, and the pairing-preserving direct-sum equivalence.
	   The dual-frame algebra step is now exact Lean:
	   `BHW.complexMinkowski_rawDualFrameIn` first extends coordinate
	   functionals from the span of an independent frame inside a nondegenerate
	   subspace and represents them by the restricted bilinear form, producing a
	   raw dual frame with the Kronecker `q`-`y` block.  Then
	   `BHW.complexMinkowski_isotropicDualFrame_of_rawDualFrame` takes a raw dual
	   frame `y` in the same subspace as the totally isotropic frame `q`, with
	   `sourceComplexMinkowskiInner d (q c) (y c') = δ c c'`, and returns
	   `qDual c = y c - (2 : ℂ)⁻¹ • ∑ k, ⟪y c, y k⟫ • q k` inside that subspace.
	   Its checked proof shows that the half-Gram subtraction preserves the
	   `q`-dual Kronecker block and kills the `qDual`-`qDual` block by symmetry
	   and total isotropy of `q`.  The checked public theorem
	   `BHW.complexMinkowski_isotropicFrame_dualFrameIn` composes these two
	   pieces, so the remaining normal-form producer may treat the isotropic
	   dual frame as an available finite-dimensional packet rather than repeat
	   the coordinate-functional and half-Gram algebra.  The residual-frame
	   wrapper `BHW.complexMinkowski_isotropicDualFrame_of_residualFrame` is also
	   checked: it applies the packet in
	   `complexMinkowskiOrthogonalSubmodule d M`, gets nondegeneracy from
	   `BHW.complexMinkowskiOrthogonalSubmodule_nondegenerate`, and converts
	   membership in that orthogonal complement into the `qDual_orth_M` and
	   `qDual_orth` fields.  The finite convergence helper
	   `BHW.tendsto_isotropicResidual_exp_neg_base` is checked as well: after
	   expanding coordinates, each finite residual sum is a sum of constants
	   multiplied by `(Real.exp (-t) : ℂ)`, hence tends to zero, and adding the
	   constant base gives convergence to `ξ`.
   ...
   | Low-rank residual-frame alignment | Selected-span frame producer, selected-span determinant-one orbit, alignment producer, residual-subspace extraction, orthogonal-complement/direct-sum packet, maximal-frame existence, basis-packaging support, quotient/rank-nullity support, proper-branch residual frame-extension packaging, residual hyperbolic basis-dual completion, and separate-frame singular contraction checked in `BHWPermutation/SourceHWLowRankAlignment.lean`, `SourceHWCommonResidualFrame.lean`, `SourceHWMaximalIsotropicFrame.lean`, `SourceHWMaximalIsotropicQuotient.lean`, `SourceHWMaximalIsotropicExtension.lean`, `SourceHWMaximalIsotropicFinal.lean`, `SourceHWResidualFrameExtension.lean`, `SourceHWResidualHyperbolicCompletion.lean`, and `SourceHWSeparateResidualFrame.lean`. | The common-frame determinant-repair branch remains checked optional support, but the active route is the direct separate-frame producer: `hw_lowRank_selectedSpanAlignment_of_selectedSpanFrame` builds the common selected base and places both residual families in the selected orthogonal complement; `complexMinkowski_maximalIsotropicFrameIn_extending` separately extends the left and right residual spans; `hw_lowRank_separateResidualFrameData` packages those frames; and `hw_sameSourceGram_singular_contractionData_of_selectedAlignment` builds the two determinant-one orbit curves required by the singular-limit API. |
   | Common isotropic frame to normal form | Checked optional consumers in `BHWPermutation/SourceHWLowRankNormalForm.lean`; no longer critical-path producer. | `BHW.hw_lowRank_commonIsotropicFrameData_of_residualAlignmentData`, `BHW.hw_lowRank_isotropicNormalForm_of_commonFrameData`, and `BHW.hw_sameSourceGram_singular_contractionData_of_commonFrameData` remain useful if a common residual frame is supplied, but the public low-rank producer now bypasses that route through separate frames. |
   | Dual frame and contraction family | Dual-frame packet, hyperbolic-block support, selected-plus-hyperbolic block map, determinant-one global extension, and packaged Lorentz family checked in `BHWPermutation/SourceHWCommonResidualFrame.lean`; consumed by `SourceHWSeparateResidualFrame.lean`. | `BHW.complexMinkowski_isotropicDualFrame_of_residualFrame` supplies dual frames for each separate residual frame, and `BHW.complexMinkowski_selectedHyperbolicContractionFamily` supplies the determinant-one null boosts.  The separate-frame producer uses this twice, once for the left residuals and once for the right residuals, so no compatible common target frame or full-block determinant branch remains on the OS-II critical path. |
...
2. `BHW.sourceExtendedTubeGramDomain_relOpen_connected`: the scalar image
   ...
   by `BHW.sourceMinkowskiGram_complexLorentzAction`.  The selected principal
   block coefficient/projection layer is now checked in
   `OSReconstruction/ComplexLieGroups/Connectedness/BHWPermutation/SourceHWSelectedProjection.lean`:
   `BHW.sourcePrincipalGramMatrix`,
   `BHW.sourcePrincipalGramMatrix_det_isUnit_of_sourceMatrixMinor_ne_zero`,
   `BHW.hw_selectedSpanCoeff`,
   `BHW.hw_selectedSpanCoeff_projection_eq`,
   `BHW.hw_selectedSpanCoeff_selected`,
   ...
   finite residual-frame support theorem is also checked:
   `BHW.hwLemma3_selectedResidual_isotropicFrameData` applies
   `Submodule.exists_fun_fin_finrank_span_eq` to the residual span, uses the
   returned membership in `Set.range residual` to inherit residual-residual
   zero and projection-residual orthogonality, uses
   `BHW.exists_coefficients_of_mem_span_finite_frame` to produce the
   coefficient functions, and closes the source decomposition with
   `BHW.hwLemma3_selectedProjection_add_residual`.  The remaining adapted
   representative construction now starts at the Hall-Wightman
   ...
   reusable variety-relative implementation interface
...
| Lemma 2, low rank | Checked and assembled: `BHW.HWSameSourceGramSingularContractionData`, `BHW.hw_sameSourceGram_singularLimit_extendF_eq`, selected-frame/alignment producers, maximal-isotropic frame extension, separate residual-frame data, `BHW.hw_sameSourceGram_singular_contractionData_of_selectedAlignment`, and the public `BHW.hw_sameSourceGram_singular_contractionData` in `SourceHWBranchLaw.lean`. | The active OS-II route no longer needs the compatible common-frame/full-block determinant repair.  Same Gram data produce the selected-span frame and determinant-one selected alignment; the left and right residual spans are separately extended to maximal isotropic frames in the selected orthogonal complement; separate dual frames and null-boost families give two determinant-one orbit curves converging to the same selected base; the analytic limit theorem then proves equality of `extendF` values. |
...
helper `BHW.continuous_wickRotateRealConfig`.  This is now checked in
`OSToWightmanLocalityOS45BHWJost.lean` by the same coordinatewise proof as
the old private helper in `OSToWightmanTubeIdentity.lean`.
...
Downstream of the compact source-patch theorem, the blueprint includes the
Lean-shaped record constructor for
...
OS-I/BHW boundary-data theorem
...
theorem is not the first active source-patch producer: its real-boundary and
lift-pairing fields may consume the finished compact source-patch theorem.  It
must not mention source representatives, EOW, PET monodromy, `bvt_W`, or final
locality.
...
edge, plus pointwise Wick/real trace formulas on the selected source patch.
It must **not** include the real-boundary comparison with the ordinary branch
or the compact lift pairing as fields of the active source-patch producer;
...
The pointwise trace comparison of `WJ (realEmbed x)` with the swapped ordinary
branch is now the active noncircular trace output: it follows from
`WJ = Dinit.branchτ` on `Dinit.Ωτ`,
`Dinit.branchτ_eq_correctedExtendF`, and the swapped real-embedding
membership `hV_swapET`.  The compact equality
`BHW.os45Figure24_sourcePatch_pairing_eq_swappedSourcePatch_of_OSI45` is then
proved from the checked `OS45SourcePatchBHWJostPairData` consumer layer and
the Euclidean-edge Wick-difference zero.  Only after that theorem is checked
may the downstream real-boundary packet build fresh zero-diagonal data for an
arbitrary compact `χ` and consume the finished compact source-patch equality.
The downstream packet must not feed back into the source-patch theorem; it must
not use scalar representatives, PET independence, or final `bvt_W` locality.
...
Lean into the direct pair-data surfaces:
...
OS-I/BHW-Jost proof that this checked difference branch is distributionally
zero on compact tests supported in the selected source patch.  The real-trace
translation and compact-pairing algebra are now checked in Lean through
`BHW.realSourceBranchDifference_zero_of_pairData_difference_zero`,
`BHW.os45CompactFigure24WickPairingEq_of_pairData_difference_zero`, and the
family/source-anchor consumers.  A separate
`OS45SourcePatchBHWJostDifferenceData` wrapper is no longer a first target
unless it carries new source provenance not already present in `P` and
`P.difference`.  Only after the public source-patch compact theorem is
assembled may the downstream full continuation package add
...
| `BHW.os45Figure24_sourcePatch_pairing_eq_swappedSourcePatch_of_OSI45` | Active route corrected; algebraic `OS45SourcePatchBHWJostPairData` consumer layer checked, analytic OS-I/BHW-Jost producer still open. | Direct OS I §4.5 / BHW-Jost compact producer.  Preferred target: construct `P : OS45SourcePatchBHWJostPairData ... (BHW.os45Figure24SourcePatch ... i hi)` for each adjacent pair.  The checked exact-source-patch consumer `os45CompactFigure24WickPairingEq_of_pairData_on_figure24SourcePatch` derives the compact Wick-pairing packet from that carrier: Euclidean-edge Wick zero, Wick/real-source integrability, `P.difference` identity propagation, and the family/source-anchor packaging are all checked.  A broader carrier on `V` remains acceptable only if it comes with canonical Figure-2-4 source-patch containment.  It must not call `BHW.os45_adjacent_commonBoundaryEnvelope`, `BHW.os45BranchHorizontalOrientedSourceGermAt_of_figure24_id`, `BHW.os45BranchHorizontalOrientedSourceGermAt_of_figure24_adjacent`, the oriented adjacent `S'_n` seed/path package, PET independence, final locality, or the later OS-specific `JostRuelleCompactBoundaryData` package that consumes this compact theorem.  The old common-boundary-envelope transcript was circular and is retained only as rejected archival material. |
...
direct OS-I §4.5/BHW-Jost source-patch compact theorem
  ...
  -> sourceDistributionalAdjacentTubeAnchor_of_pairData_difference_zero
...
1. Wick compact zero.  The compact distributional zero for the branch
   difference on the selected chart is now checked as the Euclidean-edge
   bridge below.  Its Wick integrability inputs are also checked, and the
   canonical Figure-2-4 specialization
   `BHW.integrable_wickEdge_bvt_F_mul_schwartz_on_figure24SourcePatch`
   supplies them on the final source patch.

   ```lean
   theorem BHW.os45_wickBranchDifference_zero_of_euclideanEdge_pairing_eq_on_timeSector
       [NeZero d]
             ...
             (fun x : NPointDomain d n =>
               bvt_F OS lgc n
                 (fun k => wickRotatePoint
                   (x (Equiv.swap i ⟨i.val + 1, hi⟩ k))) * χ x))
       (hid_int :
         ∀ χ : SchwartzNPoint d n,
           HasCompactSupport (χ : NPointDomain d n -> ℂ) ->
           tsupport (χ : NPointDomain d n -> ℂ) ⊆ V ->
   ...
   and the checked integrability/bookkeeping lemma
...
2. Direct BHW/Jost difference carrier.  Introduce the direct OS-I compact
   ...
   `Bord` is the ordinary BHW/Jost branch and `Btau` is the adjacent continued
   ...
   noncomputable def
       ...
       (T :
         ∀ z, z ∈ U ->
           BHW.BHWJostOrientedBranchFreeTransferNeighborhood hd n τ U z)
       (C0 : BHW.BHWJostLocalOrientedContinuationChart hd n τ U)
       (hbase : p0 ∈ Ω0 ∩ U)
       (hp0C : p0 ∈ C0.carrier)
       (start_patch : Set (Fin n -> Fin (d + 1) -> ℂ))
   ...
   The zero-step and observation constructors are now checked in Lean:
   ...
   `BHWJostOrientedTransferContinuationTrace.hasUniqueSteps_base`.  The
   checked bundled carriers are
   `BHWJostOrientedCertifiedTransferContinuationTrace` and
   `BHWJostOrientedCertifiedTransferTerminalPointTrace`; they keep the
   ordinary trace plus the step-uniqueness certificate, and they have
   `base`, `ofTracePoint`, `atEndpoint`, and forgetful maps back to the raw
   terminal-point trace API.  From this one-step uniqueness theorem, prove a
   finite induction
   `bhw_jost_orientedTransferTrace_terminalComparison_of_sameTrace`: two
   certified traces with the same finite nodes and same controlling
   branch-free neighborhoods, and with equal starting germs on an initial
   source patch, have terminal comparison data at the final node.  The
   induction state is a source patch around `chain.node i` contained in both
   current local-chart carriers, with equality of the current branches there;
   the successor case is exactly `bhw_jost_orientedTransfer_unique_next`
   applied to the stored `stepUnique j`.
   ...
   `T₁.trace.trace` and then the reversed source nodes of `T₂.trace.trace`, uses the
   ...
   equality of the initial chart of the underlying raw trace
   `T₂.trace.trace` with `B0`, is transported forward along the certified
   stored trace of `T₂` by the finite uniqueness induction,
   yielding a terminal patch inside the two observed terminal charts at `y`.  For the
   ...
   T₁.trace.trace.chain T₂.trace.trace.chain`;
   ...
   such a theorem from the current data.  The final strict-route producer should
   instead use the checked certified trace-level variant
   `BHWOrientedContinuationTraceAtlasData.ofCertifiedTerminalPointComparisonsAndInitialChart`
   whose comparison hypothesis ranges over certified terminal-point traces for
   the certified selected transfer traces.  After the trace comparison produces terminal
   ...
   The certified checked constructor
   `BHWOrientedContinuationTraceAtlasData.ofCertifiedTerminalPointComparisonsAndInitialChart`
   is now the strict-route atlas surface: it forgets certified selected traces
   to raw traces for the stored atlas, derives the selected overlap field by
   observing the certified selected traces as certified terminal-point traces,
   and proves base agreement by comparing the certified selected trace at
   `z ∈ Ω0 ∩ U` with the certified zero-step base trace observed at `z`.
   The older checked constructor
   ...
   checked accessors: `H.U_hull`, `H.U_isPathConnected`,
   `H.extendedTube_subset_ambient`, and `H.extendedTube_meets_U`.  The
   adjacent proof applies the same theorem
   ...
   `H.U_hull`, `H.U_isPathConnected`,
   `H.adjacentExtendedTubePreimage_subset_ambient`, and
   `H.adjacentExtendedTubePreimage_meets_U`; open-domain and initial
   holomorphy inputs are the checked
   `BHW.isOpen_permAct_preimage_extendedTube` and
   `BHW.differentiableOn_extendF_bvt_F_permAct_preimageExtendedTube`.
   The ordinary initial holomorphy input is the checked
   `BHW.differentiableOn_extendF_bvt_F_extendedTube`.  The real trace rewrite
   uses `BHW.permAct_realEmbed` and `H.real_tau_ET`; the adjacent Wick trace is
   not obtained from a direct
   ...
   Strict-route reset, 2026-05-09: the following oriented-continuation
   consumer paragraph is archived side infrastructure, not the active
   theorem-2 route.  The literal OS-II / OS I §4.5 implementation must not
   require `BHW.SourceOrientedScalarRepresentativeData` or
   `BHW.OS45SourcePatchBHWJostOrientedContinuationInputs` to build the compact
   Figure-2-4 branch-difference theorem; it must build the non-source-descent
   ...
   `BHW.os45Figure24SourcePatch`, obtain `Bord` and `Btau`, and call the
   checked constructor
   `BHW.OS45SourcePatchBHWJostPairData.ofHullDataAndBranches`.  This
   constructor copies `H.τ`, `H.U`, `H.V_open`, `H.V_nonempty`, `H.U_open`,
   `H.U_connected`, `H.wick_id_mem`, `H.real_id_mem`, and stores the two
   branches plus their four trace fields.  The pair-data theorem itself then
   contains no hidden analytic step.

   Production Lean has already replaced the separate difference carrier by
   methods on the checked pair carrier:

   ```lean
   def BHW.OS45SourcePatchBHWJostPairData.difference
       (P : BHW.OS45SourcePatchBHWJostPairData
         (d := d) hd OS lgc n i hi V) :
       (Fin n -> Fin (d + 1) -> ℂ) -> ℂ :=
     fun z => P.Btau z - P.Bord z

   theorem BHW.OS45SourcePatchBHWJostPairData.difference_holo
       (P : BHW.OS45SourcePatchBHWJostPairData
         (d := d) hd OS lgc n i hi V) :
       DifferentiableOn ℂ P.difference P.U

   theorem BHW.OS45SourcePatchBHWJostPairData.difference_wick_trace
       (P : BHW.OS45SourcePatchBHWJostPairData
         (d := d) hd OS lgc n i hi V)
   ...
   theorem BHW.OS45SourcePatchBHWJostPairData.difference_real_trace
       (P : BHW.OS45SourcePatchBHWJostPairData
         (d := d) hd OS lgc n i hi V)
       {x : NPointDomain d n} (hx : x ∈ V) :
...
3. Boundary uniqueness on this direct carrier.  The zero Wick distribution
   ...
   `P.difference` on the BHW/Jost hull, then to the `hzero` input consumed by
   the checked algebra.  The missing topology is explicit: `P.V_open`,
   `P.V_nonempty`, the standard real-linear Wick embedding
   `x ↦ fun k => wickRotatePoint (x k)`, and `P.wick_mem` make the Wick trace
   of `V` a nonempty open totally-real slice in `P.U`.
   ...
   `x ↦ P.difference (fun k => wickRotatePoint (x k))` on the open set `V`;
   continuity is `P.difference_holo.continuousOn.comp` with `P.wick_mem`, and
   the compact-test integral is rewritten by `P.difference_wick_trace` and
   `hwick_zero`.  Then apply the standard product totally-real identity
   theorem to the holomorphic function `P.difference` on the connected open
   set `P.U`, using `P.V_nonempty` and the
   ...
   The real-trace/distribution theorem is then the exact `hzero` shape needed
   by the checked consumer:
   ...
   Its proof calls
   `BHW.os45_pairData_difference_identity_of_wickDistributionZero`, rewrites
   the integrand to zero on `V`, and uses compact support of `ψ` to make the
   integrand zero off `V`.  The checked theorem
   `BHW.realSourceBranchDifference_zero_of_pairData_difference_zero` then
   translates this `P.difference` zero into the explicit `extendF` real-source
   branch-difference zero.  This item is an edge/totally-real uniqueness lemma
...
4. Public source-patch assembly.  The public theorem only combines items 1--3
   ...
   The Wick-side integrability hypotheses for the checked Euclidean-edge
   bridge are now checked below.  The identity Wick trace is continuous on the
   ordered real source patch by `bvt_F_holomorphic`,
   `BHW.continuous_wickRotateRealConfig`, and
   `wickRotate_mem_forwardTube_of_mem_orderedPositiveTimeSector`; compact
   support then gives integrability by
   `SCV.integrable_continuousOn_mul_schwartz_of_supportsInOpen`.  A permuted
   Wick trace is reduced pointwise to the identity Wick trace by the checked
   global permutation invariance `bvt_F_perm`.  Thus no separate unproved
   source-chart growth theorem is hidden here.

   ```lean
   theorem BHW.continuous_wickRotateRealConfig
       {d n : Nat} :
       Continuous
         (fun x : NPointDomain d n => fun k => wickRotatePoint (x k))

   theorem BHW.integrable_bvt_F_wickRotate_mul_schwartz_of_orderedSector
       [NeZero d]
       (OS : OsterwalderSchraderAxioms d)
       (lgc : OSLinearGrowthCondition d OS)
       (n : Nat)
       ...
       (hφ_supp : tsupport (φ : NPointDomain d n -> ℂ) ⊆ V) :
       Integrable
         (fun u : NPointDomain d n =>
           bvt_F OS lgc n (fun k => wickRotatePoint (u k)) * φ u)

   theorem BHW.integrable_wickEdge_bvt_F_mul_schwartz_of_orderedSector
       [NeZero d]
       (OS : OsterwalderSchraderAxioms d)
       (lgc : OSLinearGrowthCondition d OS)
       (n : Nat)
       ...
       Integrable
         (fun u : NPointDomain d n =>
           bvt_F OS lgc n
             (fun k => wickRotatePoint (u (ρ k))) * φ u)
   ```

   The public source-patch theorem is then Lean-mechanical:

   ```lean
   theorem BHW.integrable_extendF_realSource_mul_schwartz_of_ET
       [NeZero d]
       (OS : OsterwalderSchraderAxioms d)
       (lgc : OSLinearGrowthCondition d OS)
       (n : Nat)
   ...
   theorem BHW.os45Figure24_sourcePatch_pairing_eq_swappedSourcePatch_of_OSI45
     ...
     have hwickτ_int :
         ∀ χ : SchwartzNPoint d n,
           ...
           Integrable
             (fun x : NPointDomain d n =>
               bvt_F OS lgc n
                 (fun k => wickRotatePoint (x (τ k))) * χ x) :=
       BHW.integrable_wickEdge_bvt_F_mul_schwartz_of_orderedSector
         (d := d) OS lgc n hChart.V0 hChart.V0_open
         (fun x hx => hV_ordered x (hChart.V0_sub hx)) τ
     have hwickid_int :
         ∀ χ : SchwartzNPoint d n,
           HasCompactSupport (χ : NPointDomain d n -> ℂ) ->
           tsupport (χ : NPointDomain d n -> ℂ) ⊆ hChart.V0 ->
     ...
     have hid_int :=
       BHW.integrable_extendF_realSource_mul_schwartz_of_ET
         ...
         (1 : Equiv.Perm (Fin n))
         (fun x hx => by
           simpa using hV_ET x (hChart.V0_sub hx))
         ψ hψ_comp hψ_supp
     have hτ_int :=
       BHW.integrable_extendF_realSource_mul_schwartz_of_ET
         (d := d) OS lgc n hChart.V0 hChart.V0_open P.τ
         (fun x hx => by
           simpa [P.τ_eq] using hV_swapET x (hChart.V0_sub hx))
         ψ hψ_comp hψ_supp
     have hpair :=
       BHW.integral_realSourceBranchDifference_eq_zero_to_pairing_eq
   ...
   The real-source integrability helper is now checked.  It composes
   `BHW.extendF_holomorphicOn` for `BHW.extendF (bvt_F OS lgc n)` with the
   continuous real-embedding/permutation map, takes the appropriate
   extended-tube membership hypothesis as `hVρ_ET`, then applies
   ...
   branch-germ suppliers.  Together with the Wick-edge integrability helper
   above, this last step is measure algebra plus `sub_eq_zero`, not a new
   analytic theorem.
...
| `BHW.os45Figure24_sourcePatch_pairing_eq_swappedSourcePatch_of_OSI45` | Route-cycle corrected; strict-route reset applied after Claude #1821; direct producer transcript pinned, first carrier/algebra layer now checked below the public surface. | Must now construct the checked `BHW.OS45SourcePatchBHWJostPairData` carriers directly from OS I §4.5/BHW-Jost data, preferably on the exact canonical source patch `BHW.os45Figure24SourcePatch`; if the producer constructs a larger source patch, it must also prove canonical Figure-2-4 source-patch containment.  The first active Lean target is the non-source-descent local carrier `BHW.OS45BHWJostLocalContinuationData` and its producer `BHW.os45_BHWJostLocalContinuationData_from_OSI45`, not `SourceOrientedScalarRepresentativeData` and not `OS45SourcePatchBHWJostOrientedContinuationInputs`.  The checked exact-source-patch and canonical consumers then derive compact Wick equality from the carrier itself: Wick compact zero comes from the Euclidean-edge theorem, `P.difference` propagation is checked, real-boundary integration is checked, and family/source-anchor packaging is checked.  A separate `OS45SourcePatchBHWJostDifferenceData` wrapper is not a first target unless it carries additional source provenance beyond the checked pair-data fields.  It cannot use `BHW.os45_adjacent_commonBoundaryEnvelope`, the oriented branch-germ suppliers, the oriented adjacent `S'_n` seed/path package, individual real-branch `= OS.S` statements, PET independence, final locality, the quarantined source-variety descent framework, or the later OS-specific `JostRuelleCompactBoundaryData` package that consumes this compact theorem. |
...
| OS post-seed oriented corridor propagation | Assembly proof transcript pinned; production Lean not started until the oriented identity principle exists. | `BHW.os45AdjacentOrientedScalarEq_on_quarterTurnCorridor` mirrors the checked `BHW.os45AdjacentScalarEq_on_quarterTurnCorridor`: define `Φ G := hRep.Phi (sourcePermuteOrientedGram d n τ G)` and `Ψ := hRep.Phi`, obtain `hΨ_holo` by restricting `hRep.Phi_holomorphic` to `Wscal`, obtain `hΦ_holo` from checked `SourceOrientedVarietyGermHolomorphicOn.precomp_sourcePermuteOrientedGram`, subtract, prove zero on `Wseed` from `hSeed`, and apply `sourceOrientedGramVariety_identity_principle` on the connected relatively open `Wscal`.  The oriented germ API and double-domain topology are checked in `SourceOriented.lean`; the remaining reusable analytic input is the oriented source-variety identity principle, while the OS-specific inputs remain the oriented scalarization chart, compact Figure-2-4 seed, and real-seed provenance package. |
...
1. the active route is uniquely the OS II / OS I §4.5 branch-difference path:
   ...
   no alternate route is permitted unless an actual error in the OS papers is
   identified and documented locally first.  Coordinate-variety work may appear
   only as Lean implementation infrastructure for this path, not as an
...
7. the active oriented scalar-source packet separates checked source-germ
   ...
   sourced/proved;
   no new axiom may discharge any of these producers on the theorem-2 route
   unless it is first justified as a documented correction to an actual OS
   paper error.  Invariant-theory, analytic-normality, and QFT axioms are not
   valid shortcuts for the strict OS-II theorem-2 proof;
