...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   * Package `Q` as `ComplexMinkowskiMaximalIsotropicSubspaceIn d N` and apply
   ...
   The next pass should add the quotient preimage/radical-zero packet and the
   rank-nullity comparison for arbitrary isotropic `S ≤ N`.  It should not
   start by adding a new generic
