...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   | Dual frame and contraction family | Dual-frame packet plus hyperbolic-block support checked in `BHWPermutation/SourceHWCommonResidualFrame.lean`; production contraction family not started. | `BHW.complexMinkowski_rawDualFrameIn`, `BHW.complexMinkowski_isotropicDualFrame_of_rawDualFrame`, `BHW.complexMinkowski_isotropicFrame_dualFrameIn`, and `BHW.complexMinkowski_isotropicDualFrame_of_residualFrame` now produce and store `qDual_pair_zero`, `q_dual`, `qDual_orth_M`, and `qDual_orth` from nondegenerate orthogonal-complement data.  The checked hyperbolic support proves dual-pair linear independence, disjointness of `span q` from `M`, disjointness of `span qDual` from `M ⊔ span q`, disjointness of `span q` and `span qDual`, nondegeneracy of `span q ⊔ span qDual`, nondegeneracy of `M ⊔ (span q ⊔ span qDual)`, submodule scaling equivalences, two-summand scaling maps, and reciprocal-exponential determinant identities.  The remaining work in this row is to assemble the finite null boost from those checked pieces, split the ambient space by the nondegenerate block and its orthogonal complement, extend by identity, and package the resulting global map with `complexLorentzGroupOfLinearEquivPreservesInner`.  No general degenerate `complexMinkowski_wittExtension_subspaceIsometry` is used. |
