...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   `BHW.hw_lowRank_residualAlignmentData_of_selectedAlignment` must supply
   the finite-dimensional residual correction as
   `BHW.HWLowRankResidualAlignmentData A`: a determinant-one Lorentz
   transformation fixing the selected span, a common totally isotropic frame
   `q`, and span membership for the corrected left residuals and the right
   residuals.  The checked theorem
