...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   * Package `Q` as `ComplexMinkowskiMaximalIsotropicSubspaceIn d N` and apply
   ...
   The first Lean pass has now introduced the local subtype/relative-orthogonal
   helpers plus the kernel condition for quotient descent.  The next pass should
   add the quotient induced form and the two finrank identities above.  It
   should not start by adding a new generic
