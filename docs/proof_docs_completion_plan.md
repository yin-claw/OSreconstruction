...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   an injective residual embedding.  Pairing preservation for that equivalence
   is still the next block calculation, using `hE_preserves`; this support
   layer does not assert the maximal isotropic frame or the null-boost family.
