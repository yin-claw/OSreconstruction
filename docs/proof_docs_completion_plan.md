...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   `BHW.hw_lowRank_commonIsotropicFrameData_of_selectedAlignment`,
   ...
   `BHW.hw_lowRank_commonIsotropicFrameData_of_selectedAlignment` must supply
   the finite-dimensional residual correction as
   `BHW.HWLowRankCommonIsotropicFrameData A`: a determinant-one Lorentz
   transformation fixing the selected span, a common totally isotropic frame
   `q`, and explicit coefficient functions `aLeft` and `aRight` for the two
   residual families.  `BHW.exists_coefficients_of_mem_span_finite_frame`
   extracts those coefficients from the explicit span-membership fields; the
   checked consumer then constructs the dual frame, boost family, tube
   membership, and convergence fields.
   ...
   | Common isotropic frame to normal form | Checked consumer in `BHWPermutation/SourceHWLowRankNormalForm.lean`; producer still pending. | `BHW.HWLowRankCommonIsotropicFrameData` is the exact landing surface for the remaining finite-dimensional geometry: after selected-span alignment it supplies a determinant-one Lorentz correction fixing the selected block, a common totally isotropic frame `q`, and coefficient expansions for both endpoints in `span q`.  The checked theorem `BHW.hw_lowRank_isotropicNormalForm_of_commonFrameData` then constructs the dual frame, packages the determinant-one null-boost family, proves extended-tube membership by `hw_isotropicFrame_allCoefficients_mem_extendedTube`, proves convergence by `tendsto_isotropicResidual_exp_neg_base`, and returns a full `HWLowRankIsotropicNormalForm`.  `BHW.hw_sameSourceGram_singular_contractionData_of_commonFrameData` immediately converts this to the singular contraction-data API. |
