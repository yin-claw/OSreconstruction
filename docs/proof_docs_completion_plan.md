...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   * Package `Q` as `ComplexMinkowskiMaximalIsotropicSubspaceIn d N` and apply
   ...
   retyped `R`, the quotient form on `Rperp / R` is nondegenerate.  The next
   pass should prove that annihilator inclusion from nondegeneracy of `N` and
   total isotropy of `R`, then add the quotient preimage/maximal-quotient
   packet and the rank-nullity comparison for arbitrary isotropic `S ≤ N`.
