...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   * Package `Q` as `ComplexMinkowskiMaximalIsotropicSubspaceIn d N` and apply
   ...
   The remaining low-rank producer should not start by adding a new generic
   ...
   | Low-rank residual-frame alignment | Selected-span frame producer, selected-span determinant-one orbit, alignment producer, residual-subspace extraction, orthogonal-complement/direct-sum packet, maximal-frame existence, basis-packaging support, relative-orthogonal subtype/kernel helpers, quotient form, quotient radical bridge, quotient nondegeneracy, quotient-preimage carriers, retyping finrank helpers, quotient-image rank comparison, named candidate field lemmas, containment-compatible maximal subspace/frame extension, proper-branch residual frame-extension packaging, and residual hyperbolic basis-dual completion checked in `BHWPermutation/SourceHWLowRankAlignment.lean`, `SourceHWCommonResidualFrame.lean`, `SourceHWMaximalIsotropicFrame.lean`, `SourceHWMaximalIsotropicQuotient.lean`, `SourceHWMaximalIsotropicExtension.lean`, `SourceHWMaximalIsotropicFinal.lean`, `SourceHWResidualFrameExtension.lean`, and `SourceHWResidualHyperbolicCompletion.lean`. | `HWLowRankSelectedSpanFrameData` stores the scalar rank, selected principal indices, and `HWLowRankSelectedSpanFrame`.  `hw_lowRank_selectedSpanFrame_of_sameSourceGram` chooses a nonzero principal scalar block with `exists_sourcePrincipalMinor_ne_zero_of_sourceSymmetricRank`, uses the checked selected-projection theorem for head orthogonality, and uses the exact-rank Schur-zero theorem on both endpoints to obtain residual-pairing equality as `0 = 0`.  `sourceCoefficientEval_range_eq_span`, `sourceGramMatrixRank_selectedTuple_eq_of_principal_minor_ne`, and `hw_lowRank_selectedSpanFrame_detOneOrbit` reduce the selected block to the checked high-rank/Witt determinant-one orbit theorem.  `hw_lowRank_selectedSpanAlignment_of_selectedSpanFrame` builds `HWLowRankSelectedSpanAlignment`: it aligns selected spans, proves the common selected span is nondegenerate, transports the left residuals by Lorentz invariance, and places both residual families in the common orthogonal complement.  `hw_lowRank_residualSubspaces_after_selectedAlignment` extracts totally isotropic residual subspaces orthogonal to the selected block; `hw_lowRank_residualSubspaces_orthogonalComplement_after_selectedAlignment` adds inclusions into `complexMinkowskiOrthogonalSubmodule d A.M` plus disjointness from `A.M`; `complexMinkowski_totallyIsotropic_embedding_into_frame` builds the injective residual embedding into an independent isotropic frame from a dimension bound; `directSum_identity_sum_isotropicFrameEmbedding` packages the embedding, `M`-orthogonality, residual pairing preservation, and direct-sum pairing-preserving equivalence from the frame data; `coefficients_of_family_mem_span_finite_frame` extracts finite normal-form coefficients; `directSum_identity_sum_isotropicEmbedding_maps_right`/`_maps_left` prove the action on both summands; `directSum_identity_sum_isotropicEmbedding_preserves` proves full block-pairing preservation; `complexMinkowski_maximalIsotropicFrameIn_exists` proves a finite maximal totally isotropic frame exists in any ambient subspace; `complexMinkowskiMaximalIsotropicFrameIn_of_subspace_span_eq` and `complexMinkowski_maximalIsotropicFrameIn_of_subspace_containing` package a globally maximal isotropic subspace containing the right residual span into the desired frame containment; `complexMinkowskiSubmoduleIn`, `complexMinkowskiRelativeOrthogonalIn`, `complexMinkowskiRelativeOrthogonalIn_eq_orthogonal`, `complexMinkowskiSubmoduleIn_le_relativeOrthogonalIn_of_totallyIsotropic`, `complexMinkowskiSubmoduleInRelativeOrthogonal`, `complexMinkowskiSubmoduleIn_comap_le_relativeOrthogonalIn_restrict_ker`, `complexMinkowskiRelativeOrthogonal_restrict_ker_le_submoduleInRelativeOrthogonal`, `complexMinkowskiRelativeOrthogonalQuotientForm`, `complexMinkowskiRelativeOrthogonalQuotientForm_mk`, `complexMinkowskiRelativeOrthogonalQuotientForm_isSymm`, `complexMinkowskiRelativeOrthogonalQuotientForm_nondegenerate_of_restrict_ker_le`, `complexMinkowskiRelativeOrthogonalQuotientForm_nondegenerate`, and the three retyping finrank lemmas provide the checked API for the quotient/rank-nullity extension proof; the concrete quotient-image rank/nullity, maximal-bound, candidate containment, candidate ambient inclusion, candidate isotropy, raw quotient-preimage maximal extension, basis-frame containment wrapper, proper-branch frame-extension bridge, and basis-dual hyperbolic completion packet are now checked; `directSum_identity_sum_isotropicMaximalFrameEmbedding` feeds maximality directly into the residual embedding/direct-sum packet; and `hw_lowRank_residualAlignmentData_of_frameExtensionData` converts the final geometric frame-extension packet into residual-alignment data.  The remaining producer must construct the determinant-one residual hyperbolic block data: use `complexMinkowski_maximalIsotropicFrameIn_extending` to choose a maximal frame whose span contains the right residual subspace, then build the compatible source/target hyperbolic block equivalence moving the left residuals into that same span while fixing `A.M`. |
   | Common isotropic frame to normal form | Checked consumers in `BHWPermutation/SourceHWLowRankNormalForm.lean`; producer still pending. | `BHW.HWLowRankResidualAlignmentData` is now the exact landing surface for the remaining finite-dimensional geometry: after selected-span alignment it supplies a determinant-one Lorentz correction fixing the selected block, a common totally isotropic frame `q`, and span membership for the corrected left residuals and right residuals.  `BHW.hw_lowRank_commonIsotropicFrameData_of_residualAlignmentData` extracts coefficient functions and proves the endpoint expansion equations, producing `BHW.HWLowRankCommonIsotropicFrameData`.  The checked theorem `BHW.hw_lowRank_isotropicNormalForm_of_commonFrameData` then constructs the dual frame, packages the determinant-one null-boost family, proves extended-tube membership by `hw_isotropicFrame_allCoefficients_mem_extendedTube`, proves convergence by `tendsto_isotropicResidual_exp_neg_base`, and returns a full `HWLowRankIsotropicNormalForm`.  `BHW.hw_sameSourceGram_singular_contractionData_of_commonFrameData` immediately converts this to the singular contraction-data API. |
   | Dual frame and contraction family | Dual-frame packet, hyperbolic-block support, selected-plus-hyperbolic block map, determinant-one global extension, and packaged Lorentz family checked in `BHWPermutation/SourceHWCommonResidualFrame.lean`. | `BHW.complexMinkowski_rawDualFrameIn`, `BHW.complexMinkowski_isotropicDualFrame_of_rawDualFrame`, `BHW.complexMinkowski_isotropicFrame_dualFrameIn`, and `BHW.complexMinkowski_isotropicDualFrame_of_residualFrame` now produce and store `qDual_pair_zero`, `q_dual`, `qDual_orth_M`, and `qDual_orth` from nondegenerate orthogonal-complement data.  The checked hyperbolic support proves dual-pair linear independence, all disjointness facts for `M`, `span q`, and `span qDual`, nondegeneracy of `span q ⊔ span qDual` and `M ⊔ (span q ⊔ span qDual)`, submodule scaling equivalences, reciprocal-exponential determinant identities, and the hyperbolic sub-boost.  The selected-plus-hyperbolic map fixes `M`, scales `q` and `qDual`, preserves the full pairing, has determinant one, extends by identity across the orthogonal-complement splitting of the ambient space, and is packaged as `BHW.complexMinkowski_selectedHyperbolicContractionFamily` with checked vector-action formulas.  No general degenerate `complexMinkowski_wittExtension_subspaceIsometry` is used; the remaining low-rank producer work is to construct `BHW.HWLowRankResidualFrameExtensionData` from the compatible/maximal isotropic residual-frame theorem, then use the checked conversion chain to reach `BHW.HWLowRankCommonIsotropicFrameData`. |
...
| Lemma 2, low rank | Checked data/analytic support: `BHW.ComplexMinkowskiTotallyIsotropicSubspace`, `BHW.complexMinkowskiTotallyIsotropic_span_range`, `BHW.span_frame_orthogonal_to_subspace`, `BHW.HWSameSourceGramSingularContractionData`, `BHW.HWLowRankIsotropicNormalForm`, `BHW.hw_lowRank_isotropicNormalForm_to_contractionData`, `BHW.hw_sameSourceGram_singularLimit_extendF_eq`, `BHW.HWLowRankSelectedSpanFrameData`, `BHW.hw_lowRank_selectedSpanFrame_of_sameSourceGram`, `BHW.sourceGramMatrixRank_selectedTuple_eq_of_principal_minor_ne`, `BHW.hw_lowRank_selectedSpanFrame_detOneOrbit`, `BHW.HWLowRankSelectedSpanFrame`, `BHW.HWLowRankSelectedSpanAlignment`, `BHW.hw_lowRank_selectedSpanAlignment_of_selectedSpanFrame`, `BHW.hw_lowRank_residualSubspaces_after_selectedAlignment`, `BHW.complexMinkowski_maximalIsotropicFrameIn_exists`, `BHW.complexMinkowski_selectedHyperbolicContractionFamily`, `BHW.HWLowRankResidualFrameExtensionData`, `BHW.hw_lowRank_residualAlignmentData_of_frameExtensionData`, `BHW.HWLowRankResidualAlignmentData`, `BHW.hw_lowRank_commonIsotropicFrameData_of_residualAlignmentData`, and `BHW.hw_lowRank_isotropicNormalForm_of_commonFrameData`; remaining geometric producer: `BHW.HWLowRankResidualFrameExtensionData` from selected alignment, hence `BHW.HWLowRankResidualAlignmentData`, `BHW.HWLowRankCommonIsotropicFrameData`, `BHW.hw_lowRank_isotropicNormalForm_of_sameSourceGram`, and `BHW.hw_sameSourceGram_singular_contractionData`. | The residual isotropic-frame geometry, maximal-frame existence, coefficient-freedom extended-tube theorem, determinant-one null-boost Lorentz family, frame-extension-to-residual-alignment consumer, residual-alignment-to-common-data consumer, common-frame-to-normal-form consumer, and two-curve fields are decomposed into named Lean surfaces.  The low-rank finite-frame span induction, same-Gram selected-span frame producer, selected-span determinant-one alignment, residual-subspace extraction, global maximal-isotropic-frame existence, dual-frame/null-boost support, normal-form carrier, conversion to contraction data, and two-curve continuity limit inside `ExtendedTube d n` are production Lean in `BHWPermutation/SourceHWSingularLimit.lean`, `SourceHWLowRankAlignment.lean`, `SourceHWCommonResidualFrame.lean`, `SourceHWMaximalIsotropicFrame.lean`, and `SourceHWLowRankNormalForm.lean`; the remaining work is the finite-dimensional compatible/maximal common isotropic residual-frame producer that lands in `HWLowRankResidualFrameExtensionData`. |
