...
checked.  The remaining pre-envelope construction that still has to be exposed
as production Lean is the two-sided
`sliceCLM_family_from_distributionalBoundary` assembly.  It is not a new
analytic theorem from scratch: its one-sided engine
`exists_cutoffSliceCLM_family_of_boundaryValue` is already checked in
`SCV/LocalDistributionalEOW.lean`.  The assembly still has genuine content
because it must choose one cutoff window, choose the relatively compact
side-cone window generated by the fixed basis simplex, build the plus family
on that window, build the minus family on its negative image, prove the
lower-side sign/filter conversion from the original `Fminus (x - εη i)`
boundary hypothesis, and return the four evaluation/limit fields consumed by
the fixed-window family.  Do not instantiate the slice limit on the whole
ambient cone `C` from the uniform-on-compact OS-II boundary hypothesis: the
Lean route first shrinks to a conic neighborhood whose projective base has
compact closure inside `C`.
...
| `zero_not_mem_localEOWSimplexDirections`, `tendsto_neg_nhdsWithin_zero_neg_image`, `localEOW_basisSideCone_rawBoundaryValue` | The first two are checked in `SCV/LocalEOWSideCone.lean`: linear independence excludes `0` from the fixed-basis direction simplex, and continuity of negation gives the lower-side edge-filter map for `Neg.neg '' C`.  The side-cone raw boundary theorem remains pending pure local-cone/filter support for the distributional one-chart theorem: it converts OS-II uniform-on-compact direction limits to raw `nhdsWithin` limits on a relatively compact conic window generated by `localEOWSimplexDirections ys`, preventing the false shortcut of taking the raw slice limit over the whole ambient cone `C`. |
...
1. Fixed window and side cones.  Apply
   ...
   by monotonicity.
2. Choose the side cone used for the fixed window.  Let
   `K0 = localEOWSimplexDirections ys`.  It is compact and lies in `C` by
   `isCompact_localEOWSimplexDirections` and
   `localEOWSimplexDirections_subset_cone`.  Since `C` is open and `K0` is
   compact and avoids `0` by `zero_not_mem_localEOWSimplexDirections`; using
   `hK0_compact.exists_cthickening_subset_open` for `C ∩ {η | η ≠ 0}`,
   choose a thickening `K1` with compact closure inside `C`, still avoiding
   `0`, and containing `K0`; then set
   `Cplus = {y | ∃ s : ℝ, 0 < s ∧ ∃ η ∈ K1, y = s • η}` and
   `Cminus = Neg.neg '' Cplus`.  This is the relatively compact conic window
   on which the uniform-on-compact OS-II boundary hypotheses imply actual
   `nhdsWithin 0 Cplus/Cminus` slice limits.  It is not the whole ambient
   cone `C`.  The shrunken side domains are
   `Dplus = Ωplus ∩ TubeDomain Cplus` and
   `Dminus = Ωminus ∩ TubeDomain Cminus`, further intersected with the real
   translation-margin window if needed.  Their openness comes from the open
   conic construction and `IsOpen.preimage` for the imaginary-part map.  The
   fixed polywedge membership in `Dplus/Dminus` is proved from the fixed-window
   membership in `Ωplus/Ωminus` plus
   `localEOWRealLinearPart_eq_sum_smul` and the simplex normalization:
   positive chart imaginary part has direction in `K0 ⊆ K1`, while the
   negative branch applies the same argument to `-v` and then negates.
...
4. Build the two-sided slice CLMs from the distributional boundary hypotheses,
   not as assumptions.  Choose a cutoff `χ` with `χ = 1` on `B1` and
   `tsupport χ ⊆ B2`.  First derive the raw slice limits on `Cplus` and
   `Cminus`: the plus limit applies `hplus_bv` to the compact direction set
   `closure K1` and the cutoff test
   `(SchwartzMap.smulLeftCLM ℂ χ φ)`; the lower limit composes `hminus_bv`
   with the negation map from `Cminus` to `Cplus`, rewriting
   `Fminus (x + i y)` as `Fminus (x - i η)`.  Then call the checked
   one-sided theorem `exists_cutoffSliceCLM_family_of_boundaryValue` for the
   plus and minus sides and package the outputs as
   `sliceCLM_family_from_distributionalBoundary`.  The downstream complex
   boundary distribution is the cutoff pullback
   `Tcut = T.comp (SchwartzMap.smulLeftCLM ℂ χ)`, and
   `realMollifyLocal_eq_cutoffSliceCLM` gives the two evaluation fields for
   every kernel with `KernelSupportWithin ψ rψ`, because the translated
   support lies in `B1`.
...
9. Prove local covariance by `regularizedLocalEOW_pairingCLM_localCovariant`.
   ...
   `regularizedLocalEOW_family_chartKernel_covariance_on_shiftedOverlap`.
   In the nonzero support branch, the support hypotheses for `φ` and
...
7. Pinned slice-CLM production theorem for the final
   ...
   These are exactly the `hplus_bv` and `hminus_bv` arguments of
   `chartDistributionalEOW_local_envelope`, with `Traw` equal to
   `T.restrictScalars ℝ`.  The proof is fixed.  Choose an open thickening
   ...
   `localEOW_positive_imag_normalized_mem_simplex`.  The plus tendsto applies
   `hplus_bv` to `Kcl`; first prove
   ...
   * Put `tendsto_neg_nhdsWithin_zero_neg_image` and
     `localEOW_basisSideCone_rawBoundaryValue` in a new small file
     `OSReconstruction/SCV/LocalEOWSideCone.lean`, importing
     `OSReconstruction.SCV.LocalEOWChartLinear`.  This keeps the
   ...
   * Put `sliceCLM_family_from_distributionalBoundary` in a new small file
     `OSReconstruction/SCV/LocalDistributionalEOWSlice.lean`, importing
     `OSReconstruction.SCV.LocalDistributionalEOW` and
     `OSReconstruction.SCV.LocalEOWSideCone`.  Verify with:
   ...
   * After each file is checked, run
   ...
   The exact implementation transcript is:

   1. Prove the side-cone raw boundary theorem above for each cutoff test
      `(SchwartzMap.smulLeftCLM ℂ (χ : _ -> ℂ)) φ`.  Compact support of this
      test is supplied by `KernelSupportWithin.smulLeftCLM_of_leftSupport`
      from `tsupport χ ⊆ B2`; support in `E` is `B2 ⊆ E`.
   2. Define `Traw := T.restrictScalars ℝ` and
      ...
      `restrictScalars`.
   3. Call `exists_cutoffSliceCLM_family_of_boundaryValue` for each side.
   ...
   4. For evaluation, use `hDplus_sub`/`hDminus_sub` to turn
   ...
   5. For convergence, rewrite the checked one-sided limits by `hTchart`.
