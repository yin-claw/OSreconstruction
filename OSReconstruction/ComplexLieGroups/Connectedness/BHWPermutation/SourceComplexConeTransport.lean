...
/-- Centered rank-`≤` symmetric matrix cones are connected on sufficiently
small balls around zero.  This is the rank-bounded analogue of the transported
rank-exact cone theorem below. -/
theorem matrixSymmetricRankLECone_small_connected
  ...
  rcases sourceSymmetricRankLECone_small_connected
      (Fintype.card q) r hNfin_open h0Nfin with
  ...
  · have himg_conn : IsConnected
        ((fun T : Matrix (Fin (Fintype.card q)) (Fin (Fintype.card q)) ℂ =>
          ...
          (Cfin ∩ sourceSymmetricRankLEVariety (Fintype.card q) r)) := by
    ...
    · intro hS
      ...
      have hTstr' := hTstr
      rw [sourceSymmetricRankLEVariety_eq_rank_le] at hTstr'
