...
theorem d1N2PairedChartAnchorConnected_of_lightConeWitnessDiffZero
    (f : ℂ → ℂ → ℂ → ℂ → ℂ)
    (hsource : d1N2InvariantKernelSource f)
    (hdiff :
      ∀ q0 q1 p s, s ^ 2 = 4 * (p ^ 2 - q0 * q1) →
  ...
  have hf_onFT :
    ...
    (Classical.choose_spec hsource).2.2.2.2
  have hForig :
      (Classical.choose hsource) (d1N2SectionOrig q0 q1 p s v0) = f q0 q1 p s := by
    calc
      (Classical.choose hsource) (d1N2SectionOrig q0 q1 p s v0)
          = f (d1Q0 (d1N2SectionOrig q0 q1 p s v0))
              (d1Q1 (d1N2SectionOrig q0 q1 p s v0))
              (d1P01 (d1N2SectionOrig q0 q1 p s v0))
              (d1S01 (d1N2SectionOrig q0 q1 p s v0)) := by
                exact hf_onFT (d1N2SectionOrig q0 q1 p s v0) hzOrigFT
      _ = f q0 q1 p s := by
            simpa [d1InvariantQuad] using
              congrArg (fun t => f t.1 t.2.1 t.2.2.1 t.2.2.2) hquadOrig
  have hFswap :
      (Classical.choose hsource) (d1N2SectionSwap q0 q1 p s w0) = f q1 q0 p (-s) := by
    calc
      (Classical.choose hsource) (d1N2SectionSwap q0 q1 p s w0)
          = f (d1Q0 (d1N2SectionSwap q0 q1 p s w0))
              (d1Q1 (d1N2SectionSwap q0 q1 p s w0))
              (d1P01 (d1N2SectionSwap q0 q1 p s w0))
              (d1S01 (d1N2SectionSwap q0 q1 p s w0)) := by
                exact hf_onFT (d1N2SectionSwap q0 q1 p s w0) hzSwapFT
      _ = f q1 q0 p (-s) := by
            simpa [d1InvariantQuad] using
              congrArg (fun t => f t.1 t.2.1 t.2.2.1 t.2.2.2) hquadSwap
  have hkernel :
      f q0 q1 p s = f q1 q0 p (-s) := by
    exact sub_eq_zero.mp (hdiff q0 q1 p s hquad hLC hswapLC)
  calc
    (Classical.choose hsource) (d1N2SectionSwap q0 q1 p s w0) = f q1 q0 p (-s) := hFswap
    _ = f q0 q1 p s := hkernel.symm
    _ = (Classical.choose hsource) (d1N2SectionOrig q0 q1 p s v0) := hForig.symm

/-- Exact source-form equivalence (`d=1,n=2`):
light-cone witness diff-zero for the invariant kernel is equivalent to
paired-chart anchor connectivity for the sourced field. -/
theorem d1N2InvariantKernelSwapDiffZeroOnLightConeWitness_iff_pairedChartAnchorConnected
    (f : ℂ → ℂ → ℂ → ℂ → ℂ)
    (hsource : d1N2InvariantKernelSource f) :
    (∀ q0 q1 p s, s ^ 2 = 4 * (p ^ 2 - q0 * q1) →
      d1N2InvariantLightConeWitness q0 q1 p s →
  ...
  constructor
  · intro hdiff
    exact d1N2PairedChartAnchorConnected_of_lightConeWitnessDiffZero f hsource hdiff
  · intro hanchor
...
/-- In `d=1,n=2`, the EOW real-witness package is inconsistent:
no real configuration can satisfy both `x ∈ ET` and `swap·x ∈ ET` together with
the adjacent spacelike condition. -/
theorem d1N2ForwardSwapEOWGeometryPackage_false :
    ¬ d1N2ForwardSwapEOWGeometryPackage := by
  intro hgeom
  rcases hgeom with ⟨_hconn, x0, hx0_sp, hx0_ET, hx0_swapET⟩
  exact d1N2_no_real_adjacent_spacelike_witness_swap ⟨x0, hx0_sp, hx0_ET, hx0_swapET⟩
