...
private theorem onePoint_osConjTensorProduct_apply
...
private theorem onePoint_osConjTensorProduct_timeShift_apply
...
private theorem twoPoint_flattenCfg_xiShift_secondTime_eq_update
...
private theorem zero_not_mem_tsupport_translate_of_notMem
...
private theorem neg_timeShiftVec_not_mem_positive_tsupport
    (h : SchwartzSpacetime d)
...
private theorem timeShiftSchwartzNPoint_twoPointDifferenceLift_eq
  ...
  congr 1
  simpa [sub_eq_add_neg, add_assoc, add_left_comm, add_comm] using congrArg h hdiff
    ...
    by
      simpa using
        (differentiableOn_const :
          DifferentiableOn ℂ
            (fun _ : ℂ => OS.S 2 (ZeroDiagonalSchwartz.ofClassical (twoPointDifferenceLift χ h)))
            {z : ℂ | 0 < z.re}), ?_⟩
  intro t ht
...
theorem schwinger_twoPointDifferenceLift_timeShift_eq_xiShift_of_positiveSupport
  ...
  calc
    ...
    _ = ∫ y : NPointDomain d 2,
        ...
        dsimp [G]
        have hslice :=
          fromDiffFlat_update_eq_xiShift
            (k := 2) (d := d) (j := ⟨1, by omega⟩) (r := (0 : Fin (d + 1)))
        ...
        have hslice' :
            BHW.fromDiffFlat 2 d
              ...
              xiShift ⟨1, by omega⟩ 0
                (fun i => wickRotatePoint (((twoPointCenterDiffCLE d) y) i))
                ((t : ℂ) * Complex.I) := by
          simpa [BHW.fromDiffFlat_toDiffFlat] using hslice
        have hmul :
            Ψ
                (BHW.fromDiffFlat 2 d
                  (Function.update
        ...
        simpa [twoPointCenterDiff_toDiffFlat_wickRotate (d := d) y] using hmul

/-- On the admissible positive-time two-point center/difference shell, the
holomorphic existence problem can be stated either in terms of `OS.S 2` or in
terms of the explicit `ξ`-shift witness integral: the positive-real values are
already identical. -/
  ...
  have h_freq : ∃ᶠ z in nhdsWithin (1 : ℂ) {(1 : ℂ)}ᶜ, H₀ z = H₁ z := by
    ...
    have hreal_eq : H₀ ((1 + ε : ℝ) : ℂ) = H₁ ((1 + ε : ℝ) : ℂ) := by
      ...
      exact schwinger_twoPoint_xiShiftWitness_eq_of_centerIntegral_eq_of_positiveSupport
        (d := d) (OS := OS) (Ψ := Ψ) (hΨ_euclid := hΨ_euclid)
        h hh_pos (1 + ε) hpos χ₀ χ₁ (by simpa [hχ₀, hχ₁])
    exact hV_sub ⟨hz_in_V, hz_ne⟩ hreal_eq
  exact identity_theorem_connected hU_open hU_conn H₀ H₁
    hH₀_holo hH₁_holo (1 : ℂ) (by simp [U]) h_freq
              ...
              OS.S 2
                (ZeroDiagonalSchwartz.ofClassical
                  (twoPointDifferenceLift χ
                    (SCV.translateSchwartz (- timeShiftVec d t) h)))) →
          Set.EqOn H H' {z : ℂ | 0 < z.re} := by
  rcases exists_twoPoint_xiShift_holomorphicValue
      (d := d) (OS := OS) (lgc := lgc) (Ψ := Ψ) (hΨ_euclid := hΨ_euclid)
      χ₀ g hχ₀_pos hg_pos hg_compact with ⟨H₀, hH₀_holo, hH₀_real_prod⟩
  have hH₀_real_center : ∀ t : ℝ, 0 < t →
      H₀ (t : ℂ) =
        ∫ y : NPointDomain d 2,
          Ψ (xiShift ⟨1, by omega⟩ 0
            (fun i => wickRotatePoint (((twoPointCenterDiffCLE d) y) i))
            ...
            (χ₀ (y 0) * h (y 1)) := by
    intro t ht
    rw [hH₀_real_prod t ht]
    exact hmatch t ht
  intro χ
  exact exists_twoPointDifferenceLift_timeShift_holomorphicValue_of_centerValue_unique
    (d := d) (OS := OS) (Ψ := Ψ) (hΨ_euclid := hΨ_euclid)
    h hh_pos χ₀ hχ₀ H₀ hH₀_holo hH₀_real_center χ
