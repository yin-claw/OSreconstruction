...
The two-point holomorphic kernel construction is in `OSToWightmanKernel.lean`.
Base definitions (tube domains, coordinate helpers) are in `OSToWightmanBase.lean`.
-/
